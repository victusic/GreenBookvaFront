...
const Product = () => {
    const {type, category, subcategory, product, reviews, product1Line, product2Line, product3Line, product2LineName, product3LineName} = useLoaderData();
      ...
      <Suspense fallback={<Loader/>}>
        <NavigateLine>
            ...
            <Await resolve={category}>
                {(categoryName)=>(<NavigateElement route={'/category/'+categoryName[0].id}>{categoryName[0].name}</NavigateElement>)}
            </Await>
            <NavigateArrow/>
            <Await resolve={subcategory}>
                {(subcategoryName)=>(<NavigateElement route={'/subcategory/'+subcategoryName[0].id}>{subcategoryName[0].name}</NavigateElement>)}
            </Await>
        </NavigateLine>
