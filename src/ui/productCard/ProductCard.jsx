  ...
  return (
    ...
    <Link to={`/product/${product.id}`}>
      ...
      {product.manufacturer
        ?<Link to={`/product/${product.id}`} className={styles.cardTitleM}>{product.name}</Link>
        :<Link to={`/product/${product.id}`} className={styles.cardTitle}>{product.name}</Link>
      }

      <Link to={`/author/${product.autor_id}`}>
        {product.autor_name && <p className={styles.cardAuthor}>{product.autor_name + ' '}{product.autor_surname && product.autor_surname}</p>}
      </Link>
      

      <div className={styles.starsPublisherLine}>
      <Link to={`/product/${product.id}#scroll`} ><StarsPlate rate={product.review} /></Link>
        <Link to={product.publisher_id ? `/publisher/${product.publisher_id}` : `/manufacturer/${product.manufacturer_id}`}>
      ...
      <div className={styles.priceButtonsLine}>
        ...
        <div className={styles.buttonsLine}>
          {product.count > 0 && <>{favoriteTrue ? <FavoritesButtonTrue onClick={delFavorite}/> :<FavoritesButton onClick={addFavorite}/>}</>}
          {cartTrue 
            ? <BuyButtonTrue onClick={delCart}/>
            : <BuyButton disabled={!product.count && 'disabled'} onClick={addCart}>{!product.count ? 'Нет в наличии' : 'Купить'}</BuyButton>
          }
        </div>
      </div>
