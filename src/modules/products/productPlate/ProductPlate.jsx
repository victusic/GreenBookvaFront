...
import { getCountProductsList, getProductsList, getInfoProductsList } from '../../../actions/requestActions/commodity';
...
const ProductPlate = ({setTotalCount, limit, currentPage, sort, requestType}) => {
  ...
  useMemo(()=>{
    //загрузка данных
    getProductsList(requestType, id, limit, currentPage, sort, isStock, changeBindings, changeBadges, changeAuthors, changePublishers, changeManufacturers, changeMinPrice, changeMaxPrice).then((data) => {
        setProducts(data);
    });
    //загрузка кол-ва товаров
    getCountProductsList(requestType, id, isStock, changeBindings, changeBadges, changeAuthors, changePublishers, changeManufacturers, changeMinPrice, changeMaxPrice).then((data) => {
        setTotalCount(data)
    });
    //загрузка информации о продуктах
    getInfoProductsList(requestType, id, isStock, changeBindings, changeMinPrice, changeMaxPrice).then((data) => {
      setInfoProducts(data);
    });
    
  }, [limit, currentPage, sort, isStock, changeBindings, changeBadges, changeAuthors, changePublishers, changeManufacturers, changeMinPrice, changeMaxPrice, id])
    ...
    :<div className={styles.plate404}>
      {visibleNoFilter &&
         <div>
            ...
            <p className={styles.titleDesc}>{'Кажется, что под ваши критерии в фильтрах у нас товаров нет'}</p>
            <p className={styles.titleDescMin}>{'Сбросте фильтры, или уберите последнии изменения в них'}</p>
