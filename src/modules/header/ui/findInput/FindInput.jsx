...
const FindInput = ({visible, setVisible, oldFinds, setOldFinds}) => {
  ...
  return (
    <div className={styles.findPlate}>
        <input type='text' placeholder='Хочу почитать...' value={findStringInput} 
          onClick={()=> {visible ? setVisible('') : setVisible(styles.visibleNone)}} onChange={e => setFindStringInput(e.target.value)} onKeyDown={handleKeyPress} className={styles.input}
        ></input>
        <div className={styles.inputButton}><DarkButton onClick={navigate}><HandySvg src={magnifierSvg} className={styles.magnifierSvg}/></DarkButton></div>
    </div>
