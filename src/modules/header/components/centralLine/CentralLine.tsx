...
const CentralLine: React.FC = () => {
  ...
  const [oldFinds, setOldFinds] = useState<string[]>([]);

  useEffect(() => {
    if (name) {
      dispatchProfile(fetchProfileFavorites(id) as unknown as AnyAction);
      dispatchProfile(fetchProfileShoppingCart(id) as unknown as AnyAction);
      dispatchProfile(fetchProfileFavoritesList(id) as unknown as AnyAction);
      dispatchProfile(fetchProfileShoppingCartList(id) as unknown as AnyAction);
    }
  }, [name]);
