...
import { getReview, updateReview } from '../../../actions/requestActions/review';
import {
...
const ModalUpdateReview: React.FC = () => {
  const visible = useSelector((state: RootState) => state.modalVisible.modalUpdateReviewVisible);
  ...
  useEffect(() => {
    if (reviewId) {
      getReview(Number(reviewId)).then((data) => {
  ...
  function patchReview(e) {
    ...
    if (warng < 1) {
      updateReview(Number(reviewId), header, review, rating);
