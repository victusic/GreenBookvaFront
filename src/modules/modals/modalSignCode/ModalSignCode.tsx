...
const ModalSignCode: React.FC = () => {
  const visible = useSelector((state: RootState) => state.modalVisible.modalSignCodeVisible);
  ...
  function newCode() {
    setTime(60);
    dispatchSign(fetchSignGetCode(mail) as unknown as AnyAction);
  }

  function goSign(e) {
    e.preventDefault();
    dispatchSign(fetchSignPostCode(code, mail, token) as unknown as AnyAction);
  }

  useEffect(() => {
    if (profile) {
      dispatchProfile(fetchProfileBase(profile) as unknown as AnyAction);
    }
  }, [profile]);
  ...
  useEffect(() => {
    ...
    if (profileState) {
      dispatchSign(refreshSignStateAction(String(0)));
