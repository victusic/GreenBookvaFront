  ...
  return (
    <>
        <Await resolve={categories}>
            {
                (resolvedCategory)=>(
                    setCategoryProducts(resolvedCategory)
                )
            }
        ...
        <Await resolve={products}>
            {
                ...
                <div className={styles.productPlate}>
                {
                    resolvedProducts.map((product, index)=>
                        <ProductLineLocal products={product} key={index} linkId={categoryProducts ? categoryProducts[index].id : 0}>{categoryProducts ? categoryProducts[index].name : 'Загрузка'}</ProductLineLocal>
                    )
                }
                </div>
                )
