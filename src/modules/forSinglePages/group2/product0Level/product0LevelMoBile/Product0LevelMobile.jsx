...
const Product0LevelMobile = () => {
    ...
    <div className={styles.MobilePlate}>
      <Await resolve={type}>
            {
                (resolvedType)=>(                    
                    <h2 className={styles.MobileTitle}>{resolvedType[0].name}</h2>
                )
            }
        </Await>
        <MobileProductButton width={100} type={'big'} onClick={()=>setvisibleCategories(!visibleCategories)}>Категории</MobileProductButton>
                ...
                <Await resolve={categories}>
                    {
                        (resolvedCategory)=>(                    
                            resolvedCategory.map((category, index)=>
                                <Link to={'/category/'+category.id}><p className={styles.MobileCategories} key={index}>{category.name}</p></Link>
                            )
