...
const Product2LevelBar = () => {
  ...
  return (
    <ProductLeftBar>
        ...
        <Await resolve={type}>
            {(type)=>(<NavigateBarArrow route='/type/' id={type[0].id}>{type[0].name}</NavigateBarArrow>)}
        </Await>
        
        <Await resolve={category}>
            {(category)=>(<NavigateBarArrow route='/category/' id={category[0].id}>{category[0].name}</NavigateBarArrow>)}
        </Await>

        <FilterBlock params={badges} changeValue={setChangeBadgesAction} delValue={delChangeBadgesAction} value={changeBadges} addStyles={styles.antiMargin}>Статус</FilterBlock>
