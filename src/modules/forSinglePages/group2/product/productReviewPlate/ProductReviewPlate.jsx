...
import { refreshModalAddReviewAction, refreshModalSIgnVisibleAction } from '../../../../../store/modalVisibleReducer';
  ...
  return (
    <div className={styles.plate} ref={scrollRef}>
        <div className={styles.reviewTitlePlate}>
            ...
            {visibleAddReview && <div className={styles.buttonTitle}><BuyButton onClick={addReview}>Написать отзыв</BuyButton></div>}
        </div>
        {sortReviews.map((review)=><ProductReview review={review} key={review.id}/>)}
        {!fullReviews && <h5 className={styles.moreReviews} onClick={()=>setfullReviews(true)}>Показать больше отзывов</h5>}
        {fullReviews && reviews.length > 3 &&<h5 className={styles.moreReviews} onClick={()=>setfullReviews(false)}>Скрыть отзывы</h5>}
        {reviews.length < 1 &&
            <>
