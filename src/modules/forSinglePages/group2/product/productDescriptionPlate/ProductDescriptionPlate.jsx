  ...
  return (
    <div className={styles.productDescriptionPlate}>
        <div className={styles.productDescriptionLine}>
                ...
                <div className={styles.productDescriptionValue}><p className={styles.rateClick} onClick={scrollToReviews}>Оценить</p></div>
        </div>
        <h3 className={styles.productName}>{product.name}</h3>
        {product.autor_id ?
            <Link to={`/author/${product.autor_id}`}><h5 className={styles.productAuthorName}>{product.autor_name + ' '}{product.autor_surname && product.autor_surname}</h5></Link>
            :<div className={styles.productNoAuthor}/>
        }
        <ProductDescriptionLine value={product.publisher} link={`/publisher/${product.publisher_id}`}>Издательство</ProductDescriptionLine>
        <ProductDescriptionLine value={product.manufacturer} link={`/manufacturer/${product.manufacturer_id}`}>Производитель</ProductDescriptionLine>
