...
const RecommendationBanner = () => {
  ...
  return (
    <Await resolve={recommandationBanner}>
      {
        (resolvedBanner) => (
          <Link to={`/product/${resolvedBanner[0].product_id}`} className={styles.linkStyle}><img className={styles.recommendationBanner} src={recommendationRoute + resolvedBanner[0].image} alt='Рекомендуемая книга'></img></Link>
        )
    }
    </Await>
