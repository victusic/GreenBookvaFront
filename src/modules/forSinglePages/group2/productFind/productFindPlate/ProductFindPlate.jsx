...
import styles from './productFindPlate.module.scss';
import { useState } from 'react';
import { useParams } from 'react-router-dom';
import { useEffect } from 'react';
import HighPaddingButton from '../../../../../ui/buttons/highPaddingButton/HighPaddingButton';
import { useDispatch, useSelector } from 'react-redux';
import { setAuthorsAction, setBadgesAction, setBindingsAction, setManufacturersAction, setPublishersAction, setResetFilterAction, setVisibleNoFilterAction, setVisiblePagesAction } from '../../../../../store/filterProductsReducer';
import { getFindCountProductsList, getFindInfoProductsList, getFindProductsList } from '../../../../../actions/requestActions/find';

const ProductFindPlate = ({setTotalCount, limit, currentPage, sort}) => {
  ...
  useMemo(()=>{
    //загрузка данных
    getFindProductsList(findString, limit, currentPage, sort, isStock, changeBindings, changeBadges, changeAuthors, changePublishers, changeManufacturers, changeMinPrice, changeMaxPrice).then((data) => {
        setProducts(data);
    });
    //загрузка кол-ва товаров
    getFindCountProductsList(findString, isStock, changeBindings, changeBadges, changeAuthors, changePublishers, changeManufacturers, changeMinPrice, changeMaxPrice).then((data) => {
        setTotalCount(data)
    });
    //загрузка информации о продуктах
    getFindInfoProductsList(findString, isStock, changeBindings, changeMinPrice, changeMaxPrice).then((data) => {
      setInfoProducts(data);
      dispatchFilter(setVisibleNoFilterAction(true));
    });

    
    
  }, [limit, currentPage, sort, isStock, changeBindings, changeBadges, changeAuthors, changePublishers, changeManufacturers, changeMinPrice, changeMaxPrice, findString])
  ...
  useEffect(()=>{
    ...
    {products.length > 0 ? dispatchFilter(setVisiblePagesAction(true)) : dispatchFilter(setVisiblePagesAction(false))}
  }, [products])
  ...
  useMemo(()=>{
    ...
    infoProducts.forEach(product => {
      ...
      if(product.autor_id){
        ...
        let surname = '';
        if(product.autor_surname){surname = ' ' + product.autor_surname}
        {availableAutor === -1
    ...
    :<div className={styles.plate404}>
      {visibleNoFilter &&
         <div>
              ...
              <p className={styles.titleDesc}>{'Кажется, что под ваши критерии у нас товаров нет'}</p>
              <p className={styles.titleDescMin}>{'Сбросте фильтры, попробуйте найти что-нибудь другое...'}</p>
...
export default ProductFindPlate
