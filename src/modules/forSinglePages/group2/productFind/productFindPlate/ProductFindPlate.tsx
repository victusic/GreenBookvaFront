...
import {
  getFindCountProductsList,
  getFindInfoProductsList,
  getFindProductsList,
} from '../../../../../actions/requestActions/find';
...
interface ProductFindPlateProps {
...
const ProductFindPlate: React.FC<ProductFindPlateProps> = ({ setTotalCount, limit, currentPage, sort }) => {
  ...
  useMemo(() => {
    ...
    getFindInfoProductsList(findString || '', isStock, changeBindings, changeMinPrice, changeMaxPrice)
      .then((response: AnyResponse<ProductDTO[]>) => {
        if (response.status) {
  ...
  useMemo(() => {
    ...
    infoProducts.forEach((product) => {
      ...
      //авторы
      if (product.authorId) {
        const availableAutor = authors.findIndex((item) => item.id === product.authorId);
        let surname = '';
        ...
        }
        {
          availableAutor === -1
            ...
            : (authors[availableAutor].count += 1);
        }
      }
      //издательства
          ...
          {visibleNoFilter && (
            <div>
              ...
              <p className={styles.titleDescMin}>
                {'Сбросте фильтры, попробуйте найти что-нибудь другое...'}
              </p>
