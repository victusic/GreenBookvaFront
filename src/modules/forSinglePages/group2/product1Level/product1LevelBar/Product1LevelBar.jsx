...
const Product1LevelBar = () => {
  ...
  return (
    <ProductLeftBar>
        ...
        <Await resolve={type}>
            {(typeName)=>(<NavigateBarArrow route='/type/' id={typeName[0].id}>{typeName[0].name}</NavigateBarArrow>)}
        </Await>
        ...
        <Await resolve={subcategories}>
            {(resolvedCategory)=>(                    
                resolvedCategory.map((category, index)=>
                    <Link to={'/subcategory/'+category.id} className={styles.LeftBarCategories} key={index}>{category.name}</Link>
                ))
            }
        </Await>
