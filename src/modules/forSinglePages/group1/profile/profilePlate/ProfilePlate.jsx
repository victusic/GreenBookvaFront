...
import { refreshCheckCodeAction, refreshModalUpdateProfileAction } from '../../../../../store/modalVisibleReducer';
  ...
  return (
    <>
        ...
        <div className={styles.flexPad}>
            {image
                ?<img src={image.length < 26 ? avatarRoute + image : image} alt={name} className={styles.profileImg} onClick={updateProfile}></img>
                :<img src={NoAvatar} alt={'Фотография профиля отсутствует'} className={styles.profileImg} onClick={updateProfile}></img>
            }
            <div className={styles.infoPlate}>
                <div>
                    <h3 className={styles.profileName} style={{color:color}}>{name + ' ' + surname}</h3>
        ...
        <h5 className={styles.pointsMobileBlockText}>Количество баллов: <span className={styles.pointsMobileBlockCount}>{points}</span></h5>
        </div>
    </>
  )
}
