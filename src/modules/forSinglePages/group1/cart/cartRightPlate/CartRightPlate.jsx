...
import { refreshModalPayAction, refreshModalProductLimtAction } from '../../../../../store/modalVisibleReducer'
...
const CartRightPlate = ({firstPriceProducts, discountPriceProducts}) => {
    ...
    function startBuy() {
        ...
        {shopProducts.length > 12 ? dispatchVisible(refreshModalProductLimtAction(true)) : dispatchVisible(refreshModalPayAction(true))}
    }
  ...
  return (
    <>{shopProducts.length > 0 &&
        <div className={styles.cartRightPlate}>
            ...
            <div className={styles.betweenFlexPlate}>
                <PromocodeInput placeholder={'Промокод'} value={promocode} onChange={e => setPromocode(e.target.value)}/>
                <BuyButton onClick={getPromo}>Применить</BuyButton>
            </div>

            {points > 0 &&
                <Checkbox checked={spendPoints} onChange={() =>setSpendPoints(!spendPoints)}>{`Списать баллы (${points}):`}</Checkbox>
            }
            

            <p className={styles.warningText}>После данной странице могла бы быть страница с оформлением заказа, однако, поскольку это тестовый проект она была изъята, так как не имеет никакой смысловой нагрузки</p>
