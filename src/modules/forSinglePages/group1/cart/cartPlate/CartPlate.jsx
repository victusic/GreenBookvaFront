...
import { addProfileFavoriteFromListAction, delProfileFavoriteFromListAction, delProfileShoppingCartFromListAction, refreshDelFavorite, refreshDelShoppingCart, refreshProfileFavoritesMinusAction, refreshProfileFavoritesPlusAction, refreshProfileShoppingCartMinusAction, refreshShopProductsAction } from '../../../../../store/profileReducer';
import { useCurrent } from '../../../../../hooks/useCurrent';

const CartPlate = ({product, setFirstPriceProducts, setDiscountPriceProducts, firstPriceProducts, discountPriceProducts}) => {
    ...
    useEffect(()=>{
        {countProduct < 2 ? setMinusButtonStyles(styles.tileHoverDisabled) : setMinusButtonStyles(styles.tileHover)}
        {countProduct > 4 ? setPlusButtonStyles(styles.tileHoverDisabled) : setPlusButtonStyles(styles.tileHover)}
    }, [countProduct])
  ...
  return (
    <div className={styles.cartPlate}>
      <Link to={`/product/${product.id}`}><img className={styles.cartPlateImg} src={productRoute + product.image} alt={product.name}/></Link>
      <div className={styles.cartPlateOther}>
        <div className={styles.cartPlateText}>
            <Link to={`/product/${product.id}`} className={styles.cartPlateName}>{product.name}</Link>
            <Link to={product.manufacturer_id ? `/manufacturer/${product.manufacturer_id}` : `/author/${product.autor_id}`} 
                className={styles.cartPlateAuthor}>{product.manufacturer || product.autor_name + ' '}{product.autor_surname && product.autor_surname}
            </Link>
            <Link to={`/publisher/${product.publisher_id}`} className={styles.cartPlatePublisher}>{product.publisher}</Link>
            {product.count > 0 &&
                <p className={styles.cartPlateCount}>{`В наличии: ${product.count}`}</p>
            }
        ...
        <div className={styles.buttonsplate}>
            {favoriteTrue ? <FavoritesButtonTrue onClick={delFavorite}/> :<FavoritesButton onClick={addFavorite}/>}
            <CartButtonExit onClick={delFromShoppingCart}/>
        </div>
        {product.count > 0 &&
            <div className={styles.countPlate}>
                <WhiteTile addStyle={minusButtonStyles} onClick={countMinus}><h5>-</h5></WhiteTile>
                <CountInput value={countProduct}/>
                <WhiteTile addStyle={plusButtonStyles} onClick={countPlus}><h5>+</h5></WhiteTile>
            </div>
        }
