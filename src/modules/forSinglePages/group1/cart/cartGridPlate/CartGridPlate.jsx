...
const CartGridPlate = ({products, setFirstPriceProducts, setDiscountPriceProducts, firstPriceProducts, discountPriceProducts}) => {
    ...
    <div className={styles.cartPlate}>
      {finalProducts.length > 0 &&
        <>
            ...
            {manyProducts.map((product, index) => <CartPlate product={product} key={index} 
                setFirstPriceProducts={setFirstPriceProducts}  setDiscountPriceProducts={setDiscountPriceProducts} firstPriceProducts={firstPriceProducts} 
                discountPriceProducts={discountPriceProducts}/>)}
            {noProducts.length > 0 && <h3 className={styles.countTitle}>В ожидании:</h3>}
            {noProducts.map((product, index) => <CartPlate product={product} key={index}/>)}
        </>
      }
    </div>
  )
