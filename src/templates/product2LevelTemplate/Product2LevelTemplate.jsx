...
const Product2LevelTemplate = () => {
  ...
  useEffect(()=>{
    setCurrentPage(1)
  }, [limit, isStock, changeBindings, changeBadges, changeAutors, changePublishers, changeManufacturers, changeMinPrice, changeMaxPrice])
  ...
  return (
    <div className={styles.mainPlate}>
      <Await resolve={subcategoryName}>
            {
                (subcategoryName)=>(                    
                    <h2 className={styles.MobileTitle}>{subcategoryName[0].name}</h2>
                )
            }
        </Await>
      <ProductPlateTop totalCount={totalCount} limit={limit} setLimit={setLimit} setSort={setSort}/>
      <Product2LevelMobile/>
      <ProductPlate requestType={'subcategory'} setTotalCount={setTotalCount} limit={limit} currentPage={currentPage} sort={sort} />
      <ProductPagesNavigate totalCount={totalCount} limit={limit} currentPage={currentPage} setCurrentPage={setCurrentPage}/>
    </div>
  )
}
