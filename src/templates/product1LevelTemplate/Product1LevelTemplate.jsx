  ...
  return (
    <div className={styles.mainPlate}>
      <Await resolve={categoryName}>
            {
                (categoryName)=>(                    
                    <h2 className={styles.MobileTitle}>{categoryName[0].name}</h2>
                )
            }
        </Await>
      <ProductPlateTop totalCount={totalCount} limit={limit} setLimit={setLimit} setSort={setSort}/>
      <Product1LevelMobile/>
      <ProductPlate requestType={'category'} setTotalCount={setTotalCount} limit={limit} currentPage={currentPage} sort={sort} />
